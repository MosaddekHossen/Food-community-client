...
const AvailableFoods = () => {
    ...
    const handleFilter = () => {
        fetch(`https://63-community-food-sharing-server-lzsqjvm0b-mosaddek.vercel.app/food?sort=${asc}&search=${search}`)
            // fetch(`https://63-community-food-sharing-server-lzsqjvm0b-mosaddek.vercel.app/food?sort=${asc}`)
            .then(res => res.json())
            .then(data => {
                setFilterData(data)
            })
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        fetch(`https://63-community-food-sharing-server-lzsqjvm0b-mosaddek.vercel.app/food?search=${search}`)
            .then(res => res.json())
            .then(data => {
                setFilterData(data)
            })
